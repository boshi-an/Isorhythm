...
function updateLetters(circleId, inputId) {
    ...
    let circleR = Math.max(150, inputText.length * 15)
    circle.setAttribute('width', `${circleR}px`);
    circle.getElementById('height', `${circleR}px`);
    ...
    for (let i = 0; i < inputText.length; i++) {
        ...
        letterElem.style.transform = `rotate(${angle}deg) translate(${circleR}px) rotate(-${angle}deg)`;
