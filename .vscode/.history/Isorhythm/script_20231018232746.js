...
function updateLetters(circleId, inputText, bias) {
    ...
    if (circleId == "circle1") {
        ...
        for (let i = 0; i < inputText.length; i++) {
            ...
            img.src = `static/note${inputText[i]}.png`;

            img.style.transform = `translate(-50%, -50%) rotate(${angle + bias}deg) translate(${size / 2}px) rotate(${rotateAngle}deg)`;
...
function addNote(number) {
    ...
    updateLetters('circle2', talea, 180);
}

document.getElementById('input1').addEventListener('input', () => updateLetters('circle1', document.getElementById(inputId).value, 0));
